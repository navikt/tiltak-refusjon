...
class FullforGodkjenningArbeidsgiver extends React.Component<Props> {
    ...
    visBunntekstFeriedager = () => {
        if (this.state.feriedager > 0) {
            return <Normaltekst className={cls.element('marginbottom')}>
                Siden deltakeren har hatt {this.state.feriedager} feriedager, blir utgiften for disse dagene fjernet i
                grunnlaget for refusjon.
            </Normaltekst>
        }
    }

    endreFeriedager = () => {
        return ((this.state.visEndreFeriedager || this.state.feriedager > 0) &&
            <div>
                <p/>
                <Input description="Hvor mange dager har deltakeren hatt ferie i perioden?" inputMode="numeric"
                       pattern="[0-9]*"
                       bredde="S"
            ...
            <div className={cls.element('container')}>
                ...
                <Panel border className={cls.element('marginbottom')}>
                    <Normaltekst className={cls.element('marginbottom')}>
                        Har deltakeren hatt ferie i peroden {this.props.refusjon.varighet.fraDato} - {this.props.refusjon.varighet.tilDato}?</Normaltekst>
