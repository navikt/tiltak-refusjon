...
const HemmeligAdresseVarsel = (props: Props) => {
    const { aktsomhet: { diskresjonskode, kreverAktsomhet } } = props;
    ...
    return (
        <Alert variant="warning" className={styles.container}>
            <Heading spacing size="small" level="3">
                {(Diskresjonskode.STRENGT_FORTROLIG === diskresjonskode || Diskresjonskode.STRENGT_FORTROLIG_UTLAND === diskresjonskode) && (
                    <>Hemmelig adresse - Kode 6</>
                )}
                {Diskresjonskode.FORTROLIG === diskresjonskode && (
                    <>Hemmelig adresse - Kode 7</>
                )}
                {!diskresjonskode && (
                    <>Hemmelig adresse</>
                )}
