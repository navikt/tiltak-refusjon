...
const InformasjonFraAvtalenVTAO: FunctionComponent<{
    tilskuddsgrunnlag: Tilskuddsgrunnlag;
    bedriftKontonummer: string | null | undefined;
    åpnetFørsteGang?: string;
}> = (props) => {
    const avtaleLenke = `http://arbeidsgiver.nav.no/tiltaksgjennomforing/avtale/${props.tilskuddsgrunnlag.avtaleId}`;
    ...
    return (
        <Boks variant="grå">
            ...
            <IkonRad>
                ...
                <BodyShort size="small">
                    {formatterPeriode(props.tilskuddsgrunnlag.avtaleFom || '', props.tilskuddsgrunnlag.avtaleTom || '')}
                </BodyShort>
            </IkonRad>
            <VerticalSpacer rem={1} />
            <IkonRad>
            ...
            <IkonRad>
                ...
                <BodyShort size="small">
                    {formatterDato(moment(props.tilskuddsgrunnlag.tilskuddTom).add(3, 'days').toString())}
                </BodyShort>
            </IkonRad>
            <VerticalSpacer rem={1} />
            <IkonRad>
                <Label>Kontonummer:</Label>
                {props.bedriftKontonummer === null && !props.åpnetFørsteGang && <Loader type="L" />}
                {props.bedriftKontonummer && (
                    <div style={{ display: 'flex', flexDirection: 'column' }}>
                        <BodyShort size="small">{props.bedriftKontonummer}</BodyShort>
                    </div>
                )}
            </IkonRad>
            <VerticalSpacer rem={1} />
            ...
            {props.bedriftKontonummer === null && props.åpnetFørsteGang && (
                <>
                    <VerticalSpacer rem={1} />
